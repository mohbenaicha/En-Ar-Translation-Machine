...
build() {
    ...
    do
        ...
        twine upload --repository testpypi "dist/$X" -u "$USER" -p "$PASS" || 
        die "Uploading package $PACKAGE_NAME failed on file dist/$X"
    done
}
...
if [ -n "$DIRS" ]; then
    echo "...first if"
    for dir in $DIRS; do
        # only build on directory where setup.py exists
        build $dir 
    done
# if ! [ -n "$DIRS" ]; then
